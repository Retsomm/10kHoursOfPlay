  ...
  if (contacts.length === 0) {
    return (
      <div className="panel p-5 text-center space-y-2">
        <p className="font-display text-sm text-dim tracking-widest">聯盟名冊</p>
        <p className="text-sm text-dim">尚未填寫</p>
        <Link href={chapterHref} className="inline-block text-sm text-[var(--color-accent)] hover:underline">
前往建立你的聯盟 →
        </Link>
      </div>
    );
