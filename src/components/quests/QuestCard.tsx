  ...
  return (
    <div className={`panel p-5 min-w-0 space-y-3 ${quest.status === "completed" ? "panel-glow" : ""}`}>
      <div className="flex items-start justify-between gap-2">
        ...
        <span className="text-xs px-2 py-1 rounded-full border border-[var(--color-border-bright)] text-dim shrink-0">
          {QUEST_STATUS_LABEL[quest.status]}
        </span>
      </div>

      {(quest.star1_criteria || quest.star2_criteria || quest.star3_criteria) && (
        <div className="text-xs text-dim space-y-1 break-words">
      ...
      {quest.reward && <p className="text-xs text-dim break-words">獎勵：{quest.reward}</p>}

      {quest.status === "active" && (
        <div className="flex flex-wrap items-center gap-2 pt-2">
          <span className="text-xs text-dim">回報結果：</span>
          {[1, 2, 3].map((n) => (
            <button
              key={n}
              type="button"
              ...
              className="text-xs px-2 py-1 rounded border border-[var(--color-border-bright)] hover:bg-white/5 disabled:opacity-50"
            >
              {n}★
            </button>
          ))}
          <button
            ...
            className="text-xs px-2 py-1 rounded border border-[var(--color-danger)] text-[var(--color-danger)] hover:bg-white/5 disabled:opacity-50"
          >
            0★ 失敗
          </button>
        </div>
        ...
        <div className="pt-2 space-y-2">
          ...
          {!cooldownActive && (
            <button
              ...
              className="text-xs px-3 py-1.5 rounded-lg bg-[var(--color-accent)] text-[#04121f] font-bold hover:brightness-110 disabled:opacity-50"
            >
              重新開始
            </button>
          )}
      ...
      {quest.status === "active" && (
        <button
          ...
          className="text-xs text-dim hover:text-[var(--color-danger)]"
        >
          放棄這個任務
        </button>
      )}
