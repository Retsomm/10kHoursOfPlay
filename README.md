...
2. 進到專案的 SQL Editor，貼上並執行 [`supabase/schema.sql`](./supabase/schema.sql)（建立 `profiles`／`chapter_progress`／`answers`／`quests` 四張表）。這個 schema 全部用 `create table if not exists`，是冪等的（idempotent）——重複執行不會清空既有資料，只有第一次真的會建表，所以之後 `schema.sql` 有新增表（例如 Phase 3 的 `quests`）時，直接重新貼上整份執行一次即可，不用挑著跑。**它不會幫你做結構變更**：如果之後要改既有表的欄位型別或加欄位，要另外寫 `alter table` 遷移語句，不能靠重跑這份 schema。
