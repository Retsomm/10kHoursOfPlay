...
把《10,000小時的遊戲》工作手冊做成一個網站版的「英雄角色卡」：每章提供簡單／中等／困難
三種不同深度的練習，完成任一種即可通關，逐步認識自己（Phase I）、持續成長（Phase II），
資料跨裝置同步，長期用來記錄自我成長歷程。詳細規格見 [README.md](./README.md)。
...
- 全站改版成 Neon Arcade 視覺主題＋互動式首頁，章節通關規則改成「任一難度即可」，
  細節見上方 2026-09-05 條目
- **資料庫已於 2026-09-05 清空**（先前為了驗證圖表元件塞的測試假資料已全部移除），
  現在是全新、空白的狀態，等使用者本人以真實內容填答、當真正的使用者實測

**下一步（依序）：**
1. 使用者以真實使用者身分完整跑一輪：登入、填答各章節（可以只填一種難度）、看英雄狀態
   儀表板、任務追蹤器，回報體驗與 bug
2. 視使用狀況決定要不要做 Phase 3 的選配延伸（刻意練習時數記錄、課責提醒 UI）
3. 正式站目前用 Clerk Development instance（`pk_test_`，有使用量限制），若之後要開放
...
## 2026-09-05 — 章節通關規則改成「任一難度即可」，全站 Neon Arcade 視覺重構

**通關規則變更（重要，改到核心資料模型）**：使用者回饋「簡單/中等/困難是同一主題的三種
深度練習，不是彼此的先修關卡，不該逼使用者三個都填完才算數」。改動：
- 章節通關＝完成任一難度（`src/lib/progress.ts` 的 `isChapterComplete`），拿掉三層依序
  解鎖的限制，三個難度從一開始就都能填、沒有先後順序。刪除 `LockedTierPanel.tsx`（不再
  需要「鎖定中」畫面）。
- 英雄等級、Phase 完成度等全站聚合統計，從「已完成關卡數 / 章節數×3」改成「已通關章節數 /
  章節數」（`heroStatus.ts` 的 `HeroLevelInfo` 欄位改名 `totalTiers/totalCompleted` →
  `totalChapters/completedChapters`）。
- **已知取捨**：英雄狀態儀表板有幾個圖表（天賦金字塔／技能金字塔／技能網雷達圖／聯盟名冊／
  人生旅程的里程碑與五年目標／六步驟對齊雷達圖）是讀取「特定章節的特定難度」的答案
  （例如天賦金字塔只認 Ch3-1 的困難關）。章節放寬成任一難度即可通關後，使用者可能只填了
  簡單關，導致「章節顯示已通關，但某個圖表還是空的」——這是使用者已經確認接受的取捨，
  因應方式是在那些關鍵難度的填答頁面加上提示文字（`TierContent.dashboardHint`），
  提醒使用者「想在總覽看到 OOO，要填這一關」。
- 六步驟旅程（`SixStepRoadmap`）先前才因為「一步驟合併兩章節變成 X/6 關卡看不懂」被
  改成逐章節分開顯示，這次順勢把底層 `SixStepProgress.done/total` 也從關卡數改成
  章節數，語意才真正一致。

**全站 Neon Arcade 視覺重構**（另見 git commit 29ed87b/332ac43/fab7a38/a3783d7/0cf35e1）：
像素字體、CRT 掃描線、發光邊框，首頁改寫成完全互動的單頁式介紹（sticky HUD、六步驟節點、
NPC/HERO 對比滑桿、關卡選單、響應式等級路線、開始旅程全螢幕過場動畫），PWA manifest／
apple-icon 補齊、Clerk 版面與配色踩坑修復（`cardBox` 未約束寬度、`colorNeutral` 未設定
導致選單看不清楚、已登入時 `<SignIn/>` 自帶重新導向會繞回首頁耗時 3 秒）。

**資料庫已清空**（2026-09-05，使用者要求）：先前為了驗證圖表元件塞的測試假資料
（profiles/chapter_progress/answers/quests 四張表）已全部清除，之後由使用者本人
以真實內容測試、除錯。
